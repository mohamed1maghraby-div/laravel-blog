...
@section('content')
    ...
    @include('admin.post_comments.filter.filter')
        ...
        <table class="table table-hover">
            ...
            <tbody>
                @forelse ($comments as $comment)
                    <tr>
                        <td><img src="{{ get_gravatar($comment->email, 50) }}" class="img-circle" alt="{{ $comment->name }}"></td>
                        <td><a href="{!! $comment->url != '' ? $comment->url : 'javascript(0);' !!}" target="_blank">{{ $comment->name }}</a>{{ $comment->user_id != '' ? '(Member)' : '' }}</td>
                        <td>
                            {!! $comment->comment !!}
